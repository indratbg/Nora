...
@section('content')
    ...
    <form class="form-horizontal" action="{{ url('admin/update_article/'.$data->id) }}" method="post"
        ...
        <div class="form-group <?php if ($errors->has('category')) echo 'has-error' ?>">
            ...
            <div class="col-sm-5">
                <select name="category" class="form-control">
                    <option value="" @if($data->category =='')selected="selected" @endif >--Select Category--</option>
                    @foreach(App\Category::whereCategory_id1('article')->get() as $row )
                        <option value="{{ $row->category_id2 }}" @if( $data->category == $row->category_id2 )selected="selected" @endif >{{ Ucfirst($row->category_name) }}</option>
                    @endforeach
                </select>
        ...
        <div class="form-group <?php if ($errors->has('post_at')) echo 'has-error' ?>">
            ...
            <div class="col-sm-2">
                <input type="text" class="form-control datepicker" name="post_at" value="{{ $data->post_at }}"
        ...
        <div class="form-group <?php if ($errors->has('body')) echo 'has-error' ?>">
            <div class="col-sm-10">
                <textarea class="form-control ckeditor" name="body">{{ $data->body }}</textarea>
                @if ($errors->has('body'))
                    <span class="help-block">

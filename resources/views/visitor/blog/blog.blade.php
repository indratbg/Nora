...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-sm-10">
            <div class="posts">
                @foreach($data as $row)
                    <h2 class=""><a href="{{ url('blog_detail/'.$row->id) }}" class="link"> {{ $row->title  }}
                            <small>{{ '&emsp;'.$row->subtitle  }}</small>
                        </a></h2>
                    <small><span
                                class="fa fa-clock-o">&nbsp;Created at &nbsp;</span>{{ $row->created_at->diffForHumans() }}
                    </small>
